...
- 新增健康信息卡应用（`demo_health`）：四页式 ICE/医疗信息展示（概览、紧急联系、医疗信息、急救须知），数据来自可编辑的 `main/health_profile.h` / `main/health_profile.c`；配套由 `tools/gen_health_font.ps1` 生成的 Noto Sans CJK SC 子集中文字体、`Health` 菜单入口，以及档案数据主机测试（`tests/test_health_profile.c`）。`ui_pixel.c` 的屏幕标题改用中文字体渲染，中文标题可正常显示。

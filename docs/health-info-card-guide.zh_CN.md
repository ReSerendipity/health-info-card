...
- 二维码存放在独立 `imgstore` 数据分区（128 KB），划分为 **4 个槽位**：
  前 3 槽各 32 KB、第 4 槽 28 KB，头区 4 KB 记录每槽长度。
- 单张二维码上限 **28 KB**（`JPEG_STORE_SLOT_MAX`）。4 槽合计约 112 KB，
  占数据区约 **90% 预算**，刻意不用满：预留余量防止碎片、写入抖动或影响
  NVS 等其他存储。28 KB 对 240×240 JPEG（约 10–20 KB）绰绰有余。
- 门户提供 4 个上传位（家属 / 备用 / 社区·医生 / 其他），设备上翻页键逐张切换，
  页码自动显示为 `4+N`（N 为已上传槽位数），按确认键全屏查看当前二维码。

...
- Added the Health Info Card application (`demo_health`): a four-page ICE/medical-info display (overview, emergency contacts, medical information, first-aid notes) driven by the editable `main/health_profile.h` / `main/health_profile.c` profile, with Noto Sans CJK SC subset fonts generated by `tools/gen_health_font.ps1`, a `Health` menu entry, and a host-side profile test (`tests/test_health_profile.c`). `ui_pixel.c` now renders screen titles with the CJK font so Chinese titles display correctly.

...
static const demo_entry_t DEMOS[] = {
    { "Health", demo_health_enter, demo_health_exit, demo_health_key },
...
void app_main(void) {
    ...
    s_ok[0] = true;                                   // Health 仅需显示+按键,已确认可用
    ...
    // 串口截屏 + 按键注入(真机验收用):按键走统一分发 on_key,保持菜单状态机一致
    fap_screenshot_set_key_cb(on_key);
